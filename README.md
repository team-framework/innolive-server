...
액세스 토큰의 subject가 이미 삭제된 사용자를 가리키면 응답 유실 뒤 안전하게 재시도할
수 있도록 동일하게 `204 No Content`를 반환합니다. 이 조정 경로에서는 만료된 토큰도
서명·issuer·audience와 원래 유효 기간을 검증한 뒤, 사용자 행이 이미 없을 때만 성공으로
처리합니다. 만료된 토큰이 활성 계정을 삭제할 수는 없으며 잘못된 토큰은 계속
`401 Unauthorized`를 반환합니다.
