...
type Config struct {
	...
	ReferenceDecodeConcurrency int
	FFmpegPath                 string
...
func Load() (Config, error) {
	cfg := Config{
		...
		ReferenceDecodeConcurrency: envInt("REFERENCE_DECODE_CONCURRENCY", 4),
...
func (c Config) Validate() error {
	...
	if c.ReferenceDecodeConcurrency < 0 {
		return errors.New("REFERENCE_DECODE_CONCURRENCY must not be negative")
	}
