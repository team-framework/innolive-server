...
순서로 정리한 뒤 마지막 데이터베이스 트랜잭션에서 계정 행을 삭제합니다. 유효한
액세스 토큰의 subject에 해당하는 사용자 행이 이미 없으면 외부 정리를 다시 실행하지
않고 응답 유실 재시도를 `204 No Content`로 조정합니다. 이 조정 경로에서는 현재 시각에
실제로 만료된 토큰만 서명·issuer·audience와 원래 유효 기간을 다시 검증합니다. 활성
사용자에게 제시된 만료 토큰, 아직 유효하지 않은 토큰, 잘못된 토큰, 그리고 행이 남은
비활성 사용자는 `401 Unauthorized`를 반환합니다. 사용자 상태 조회가 실패하면 원인을
서버 로그에 기록하고 재시도 가능한 `503 Service Unavailable`을 반환합니다.
